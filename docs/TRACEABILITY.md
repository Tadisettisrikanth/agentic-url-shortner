...
| REQ-4F | Bounded retry, fallback, rollback and safe stop | Fixed validation capabilities, classified retries, repair adapter, safe stop and verified baseline rollback C6; graph orchestration C7 | Capability-policy, API attempt persistence and rollback verification tests C6 | Persisted attempts, decisions, audit events and rollback manifest evidence | PARTIAL-C6 |
...
| REQ-6 | Risks, trade-offs, failure scenarios and safety validation | Real fixed-capability validator, bounded evidence, failure classification and governed repair C6; risk policies C7 | Successful build evidence, credential removal and terminal rollback tests C6 | Attempt output, classification, repair lineage and rollback records | PARTIAL-C6 |
